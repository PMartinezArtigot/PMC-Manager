...
func _ready():
	...
	idk = (snapped((totalHC / Global.missionlist[Global.arr[2]][12]), 0.1)) * 0.8 * 100
	if (idk > 100):
		idk = 100
...
func _on_startmission_pressed():
	Global.ammo = Global.ammo - Global.missionlist[Global.arr[2]][0]
	Global.ration = Global.ration - Global.missionlist[Global.arr[2]][1]
	Global.constmats = Global.constmats - Global.missionlist[Global.arr[2]][2]
	Global.parts = Global.parts - Global.missionlist[Global.arr[2]][3]
	Global.money = Global.money - Global.missionlist[Global.arr[2]][4]
	hide()
	missioninprogress()
		
func missioninprogress():
	var winchance = rng.randi_range(0, 100)
	if (Global.missionlist[Global.arr[2]][11] == 0):
		if (winchance <= idk):
			Global.ammo = Global.ammo + Global.missionlist[Global.arr[2]][5]
			Global.ration = Global.ration + Global.missionlist[Global.arr[2]][6]
			Global.constmats = Global.constmats + Global.missionlist[Global.arr[2]][7]
			Global.parts = Global.parts + Global.missionlist[Global.arr[2]][8]
			Global.money = Global.money + Global.missionlist[Global.arr[2]][9]
	else:
		Global.mission3inprog = 1
		if (winchance <= idk):
			Global.currentmissions.append([Global.missionlist[Global.arr[2]][5], Global.missionlist[Global.arr[2]][6],
			Global.missionlist[Global.arr[2]][7], Global.missionlist[Global.arr[2]][8], Global.missionlist[Global.arr[2]][9],
			Global.missionlist[Global.arr[2]][11], 3])
		else:
			Global.currentmissions.append([0, 0, 0, 0, 0, Global.missionlist[Global.arr[2]][11], 3])
